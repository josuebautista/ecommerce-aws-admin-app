...
const Categories = () => {
  ...
  const deleteSingleProperty = (indexToRemove) => {
    ...
    const newProperties = properties.filter((property, index) => index !== indexToRemove);
          ...
          <div>
            {
              !!properties && properties.map((property, index) => (
                <div key={index} className='w-full flex gap-1'>
                  <input type='text' value={property.name} onChange={(ev) => handlePropertyNameChange(index, property, ev.target.value)} className='w-1/2' placeholder='Name' />
                  <input type='text' value={property.values} onChange={(ev) => handlePropertyValuesChange(index, property, ev.target.value)} className='w-1/2' placeholder='Value' />
                  <button onClick={() => deleteSingleProperty(index)} className='bg-slate-500 hover:bg-slate-400 transition duration-200 text-white px-2 my-2 rounded-lg'>Remove</button>
                </div>
              ))
            }
          </div>
